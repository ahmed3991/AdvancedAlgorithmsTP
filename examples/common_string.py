...
# this is function lcs_recursive_without_memo
...
# this is function lcs_recursive_with_memo
...
# Generator the character
class StringGenerator:
    def __init__(self, alphabet=['A', 'B', 'W' , 'Z']):
...
def save_to_csv(results, filename="results.csv"):
    headers = ["Function", "Generated String", "Time", "Memory Usage", "Time Complexity (O)"]
...
def compare_memory():
    ...
    results.append(["Dynamic Programming", x20, f"{dynamic_time:.4f} seconds", f"{dynamic_memory} bytes", "O(n * m)"])

    start_time = time.time()
    lcs_dynamic(x10, y)
    dynamic_time = time.time() - start_time
    dynamic_memory = memory_usage()
    results.append(["Dynamic Programming", x10, f"{dynamic_time:.4f} seconds", f"{dynamic_memory} bytes", "O(n * m)"])
    ...
    results.append(["Recursive with Memoization", x10, f"{recursive_memo_time:.4f} seconds", f"{recursive_memo_memory} bytes", "O(n * m)"])
    ...
    recursive_memo_time = time.time() - start_time
    recursive_memo_memory = memory_usage()
    results.append(["Recursive with Memoization", x20, f"{recursive_memo_time:.4f} seconds", f"{recursive_memo_memory} bytes", "O(n * m)"])
    ...
    results.append(["Recursive without Memoization", x10, f"{recursive_time:.4f} seconds", f"{recursive_memory} bytes", "O(2^n)"])

    start_time = time.time()
    lcs_recursive_without_memo(x20, y, len(x20), len(y))
    recursive_time = time.time() - start_time
    recursive_memory = memory_usage()
    results.append(["Recursive without Memoization", x20, f"{recursive_time:.4f} seconds", f"{recursive_memory} bytes", "O(2^n)"])
