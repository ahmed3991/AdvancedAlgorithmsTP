...
def save_to_csv(results, filename="results.csv"):
    headers = ["Method", "Generated String", "Time", "Memory Usage", "Time Complexity (O)"]
...
def compare_memory():
    ...
    results.append(["Recursive without Memoization", x10, f"{recursive_time:.6f} seconds", f"{recursive_memory} bytes", "O(2^n)"])
    ...
    results.append(["Recursive with Memoization", x10, f"{recursive_memo_time:.6f} seconds", f"{recursive_memo_memory} bytes", "O(n * m)"])
    ...
    results.append(["Dynamic Programming", x10, f"{dynamic_time:.6f} seconds", f"{dynamic_memory} bytes", "O(n * m)"])
    ...
    recursive_time = time.time() - start_time
    recursive_memory = memory_usage()
    results.append(["Recursive without Memoization", x20, f"{recursive_time:.6f} seconds", f"{recursive_memory} bytes", "O(2^n)"])
    ...
    recursive_memo_time = time.time() - start_time
    recursive_memo_memory = memory_usage()
    results.append(["Recursive with Memoization", x20, f"{recursive_memo_time:.6f} seconds", f"{recursive_memo_memory} bytes", "O(n * m)"])

    start_time = time.time()
    lcs_dynamic(x20, y)
    dynamic_time = time.time() - start_time
    dynamic_memory = memory_usage()
    results.append(["Dynamic Programming", x20, f"{dynamic_time:.6f} seconds", f"{dynamic_memory} bytes", "O(n * m)"])
