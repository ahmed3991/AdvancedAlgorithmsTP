...
def measure_performance(lcs_func, *args):
    ...
    mem_usage_res = memory_usage((lcs_func, args))
    end_time = time.time()
...
def main():
    ...
    for length_X, length_Y in [(5, 10), (15, 20)]:
        ...
        results.append(["Recursive (No Memo)", length_X, length_Y, time_rec, mem_rec, lcs_rec])
        results.append(["Recursive (With Memo)", length_X, length_Y, time_rec_memo, mem_rec_memo, lcs_rec_memo])
        results.append(["Dynamic Programming", length_X, length_Y, time_dp, mem_dp, lcs_dp])

    headers = ["Method", "Length of X", "Length of Y", "Time (Seconds)", "Memory Usage (MB)", "Longest Common Subsequence"]
