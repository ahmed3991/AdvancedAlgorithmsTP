...
def merge_sort(arr):
    if len(arr) > 1:
        ...
        left_half = arr[:mid]  # Divide the elements into 2 halves
        right_half = arr[mid:]

        merge_sort(left_half)  # Recursive call to sort the first half
        merge_sort(right_half)  # Recursive call to sort the second half
    ...
    return Metrics(len(arr), 0, 0)  # Placeholder return as no comparisons or moves are tracked here
