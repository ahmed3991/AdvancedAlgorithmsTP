...
class ComplexityVisualizer:
    ...
    def plot(self, best_fit_function: str, title: str = "Complexity Function Fit"):
        ...
        best_function = next(
            f for f in self.complexity_functions if f.name() == best_fit_function
        )
        plt.plot(
            self.x,
            best_function.evaluate(self.x),
            color="red",
            label=f"Best fit: {best_fit_function}",
            linewidth=2,
        )
...
class ComplexityDashboardVisualizer:
    ...
    def plot_dashboard(self):
        ...
        for i, (algorithm, group_by_algorithm) in enumerate(
            self.grouped_data.groupby("algorithm")
        ):
            for j, (data_type, group_by_data_type) in enumerate(
                group_by_algorithm.groupby("data_type")
            ):
                ...
                best_fit_function_name, best_fit_function = (
                    self.complexity_analyzer.get_best_fit(x, y)
                )
                ...
                ax.plot(
                    x,
                    best_fit_function.evaluate(x),
                    color="red",
                    label=f"Best fit: {best_fit_function_name}",
                    linewidth=2,
                )
